...
│
├── app/
│   └── streamlit_app.py    # Analyst dashboard — 7 tabs (scoring, audit log, analytics, etc.)
│
├── data/
│   ├── raw/                # Original Kaggle dataset
│   └── processed/          # Engineered features used for training/serving
│
├── notebooks/
│   ├── 01_eda.ipynb                  # Exploratory data analysis
│   ├── 02_feature_engineering.ipynb  # Feature creation, scaling
│   └── 03_modeling.ipynb             # Random Forest + Isolation Forest training, tuning
│
├── reports/                # Saved plots (e.g. calibration curve)
│
├── src/
│   ├── scoring.py           # Shared RF+ISO blend logic — used by API, dashboard, and tests
│   ├── llm_narrative.py     # Gemini-powered plain-English explanations
│   ├── data_pipeline.py     # Loads/processes data into PostgreSQL
│   └── extract_samples.py   # Generates curated demo transactions for the dashboard
│
├── tests/
│   ├── test_finguard.py    # Unit tests for scoring/threshold logic
...
```
Data → SQL → Feature Engineering → Machine Learning → SHAP Explainability →
Anomaly Detection → Risk Decision → Streamlit Dashboard → FastAPI API → Automated Testing
```
